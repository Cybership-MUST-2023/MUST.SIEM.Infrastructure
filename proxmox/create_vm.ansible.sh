...
# Cloud init
SSH_USER=must
SSH_PRIV_KEYS=( /root/.ssh/MUST.Ansible /root/.ssh/MUST.Backup )   # Private SSH keys to be placed on the Ansible VM (stored on Proxmox server)
SSH_KEYS_ALLOWED=( /root/.ssh/MUST.Proxmox.pub )                    # SSH keys allowed to connect to the Ansible server

ANSIBLE_SCRIPTS_DIR=/root/ansible-scripts
ANSIBLE_SCRIPTS=( setup_ansible.sh setup_ansible_vault.sh )
...
for key in "${SSH_KEYS_ALLOWED[@]}"
do
    qm set $VM_ID --sshkeys $key
done

echo "Resizing disk... (if this takes forever, something has gone wrong. CTRL + C recommended)"
