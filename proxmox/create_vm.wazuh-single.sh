...
VM_ID=210
COUNT=1
NAME=must-wazuh-single
DESC="MUST Wazuh stack (indexer, manager and dashboard) VM"
...
# Required for disk size calculation
MONITORED_WORKSTATIONS=1
MONITORED_SERVERS=0
MONITORED_NETWORK_DEVICES=0

MONITORED_WORKSTATIONS_GB=$((0,1*$MONITORED_SERVERS))
MONITORED_SERVERS_GB=$((0,04*$MONITORED_WORKSTATIONS))
MONITORED_NETWORK_DEVICES_GB=$((0,2*$MONITORED_NETWORK_DEVICES))

REQUIRED_DISK_SIZE=$(($MONITORED_WORKSTATIONS_GB+$MONITORED_SERVERS_GB+$MONITORED_NETWORK_DEVICES_GB))
if [ "$REQUIRED_DISK_SIZE" -gt 8 ]; then
   DISK_SIZE="${REQUIRED_DISK_SIZE}G"
else
   DISK_SIZE="8G"
fi
